...
export default function ShoppingTab({ state, t = (key) => key, language = "pt" }) {
  const rows = getShoppingRows(state, language);
  return (
    <section className="tab-panel active">
      <div className="toolbar">
        <div><h3>{t("shopping.title")}</h3><p>{t("shopping.subtitle")}</p></div>
        <button className="secondary-button" type="button" disabled={!rows.length} onClick={() => navigator.clipboard.writeText(rows.map((row) => `${row.name}\t${formatQty(row.toBuy)}\t${row.unit}`).join("\n"))}>{t("shopping.copy")}</button>
      </div>
      {!rows.length && <p className="empty-state">{t("shopping.emptyCart")}</p>}
