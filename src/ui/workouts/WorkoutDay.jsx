...
export default function WorkoutDay({ day, dispatch }) {
  ...
  return (
    <article className="workout-day-card">
      <header>
        <div><strong>Dia {day.label} - {day.title}</strong><span>{day.exercises.length} exercício(s)</span></div>
        <div className="toolbar-actions">
          <button className="secondary-button" type="button" onClick={() => dispatch({ type: "workout/session-start", dayId: day.id })}>Fazer</button>
          <button className="secondary-button" type="button" onClick={() => dispatch({ type: "workout/exercise-add", dayId: day.id })}>Exercício</button>
        </div>
      </header>
      ...
      <div className="workout-exercise-list">
        ...
        {!day.exercises.length && <p className="empty-state">Nenhum exercício neste dia.</p>}
      </div>
    </article>
  );
}
