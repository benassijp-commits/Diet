...
export default function WorkoutsTab({ state, dispatch, notify, t = (key) => key }) {
              ...
              {(state.workoutLogs || []).slice(-8).reverse().map((session) => (
                <div className="alert-item workout-history-item" key={session.id}>
                  <button type="button" className="history-toggle" onClick={() => setExpandedLog(expandedLog === session.id ? "" : session.id)}>
                    ...
                    <span>{new Date(session.finishedAt || session.startedAt).toLocaleDateString("pt-BR")} | {session.setLogs?.length || 0} {t("workouts.sets")} | {session.durationSeconds ? formatTimerSeconds(session.durationSeconds) : "-"}</span>
                  </button>
        ...
        <div className="workout-main">
          <div className="toolbar">
            ...
            <div className="toolbar-actions">
              ...
              <button className="secondary-button" type="button" onClick={() => confirm("Excluir treino ativo?") && dispatch({ type: "workout/delete-active" })}><Trash2 size={16} /> {t("workouts.delete")}</button>
            </div>
          </div>
